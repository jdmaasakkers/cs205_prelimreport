	...
	for reg in range(len(lambdas)):
		...
		if rank == 0: 
			...
			# for i in xrange(1, size):
				# rank, size, iw, iout, iclass = comm.recv(source=i,tag=1)
				# ws[rank-1,:] = iw
				# iouts[rank-1,:] = iout
				# iclasses[rank-1,:] = iclass

		else:
			for i in range(Nt):
				y_tr_map[i] = label_func(train_labels[i],rank-1)
		...
		ranks, iws, iclass_s =  comm.gather((rank, iw, iout, iclass), dest=0,tag=1)
		comm.Barrier()
		comm.Disconnect()
